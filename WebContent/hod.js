...
function report(event) {
    ...
    if(event.srcElement.id=='submit-Report') {
        ...
        if(confirm("Are you sure you want to Delete your report on "+personalReportDate+" during hour "+hour+" from the database")) {
            document.querySelector('.report-section').classList.toggle('translate-report');
         setTimeout(function() {
...
function submitReport() {
    ...
    xmlrequest("reportservlet?id=submit&date="+date+"&year="+year+"&department="+department+"&hour="+hour+"&lecture="+lecture,printResponse);
...
function fetchPersonalReport() {
            ...
            xmlrequest("reportservlet?id=fetchPersonal&date="+date,printFetchPersonalReport);
...
function submitStatus() {
            ...
            xmlrequest("reportservlet?id=status&date="+date+"&year="+year+"&status="+status,printResponse);
...
function printFetchReport(response) {
 ...
 function printFetchPersonalReport(response) {
    ...
    if(obj.length>0) { 
            ...
            for(var i=0;i<obj.length;i++) {
                table+="<tr id=\"personal-response-row-"+obj[i].hour+"\"><td>"+obj[i].hour+"</td><td>"+obj[i].workDone+"</td><td>"+obj[i].updatedOn+"</td><td>"+obj[i].year+"</td><td>"
                +obj[i].department+"</td><td><button id=\"personal-response-edit-"+(obj[i].hour)+"\">Edit</button><button id=\"personal-response-delete-"+(obj[i].hour)+"\">Delete</button></td></tr>";
            }
            table+="</table>";
